...
const ASSETS = [
  ...
  '/games/cell.html', '/games/chicken.html', '/games/dungeon.html', '/games/fishing.html',
...
self.addEventListener('fetch', e => {
  ...
  // HTML、遊戲資料檔（assets/data/*.js）、遊戲設定檔（/games/xxx-config.js 等）：網路優先，永遠拿最新版
  const isGameConfig = /\/games\/[^\/]+\.js(\?|$)/.test(req.url);
  if (req.headers.get('accept')?.includes('text/html') || req.url.includes('/assets/data/') || isGameConfig) {
    e.respondWith(
      fetch(req)
        .then(res => {
